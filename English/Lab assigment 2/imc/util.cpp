...
double *util::minDatasetInputs(Dataset *dataset)
{
    ...
    for (int i = 0; i < dataset->nOfInputs; i++){
        mins[i] = dataset->inputs[0][i];
        for (int j = 1; j < dataset->nOfPatterns; j++){
            if (dataset->inputs[j][i] < mins[i])
                mins[i] = dataset->inputs[j][i];
        }
    }
    return mins;
}
...
double *util::maxDatasetInputs(Dataset *dataset)
{
    ...
    for (int i = 0; i < dataset->nOfInputs; i++){
        maxs[i] = dataset->inputs[0][i];
        for (int j = 1; j < dataset->nOfPatterns; j++){
            if (dataset->inputs[j][i] > maxs[i])
                maxs[i] = dataset->inputs[j][i];
        }
    }
    return maxs;
}
